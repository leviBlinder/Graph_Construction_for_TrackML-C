#include "geometric_functions.h"
  ...
  geometric_functions::geometric_functions(double _pt_min, size_t _n_eta_sections, size_t _n_phi_sections, pair<double,double> _eta_range, pair<double,double> _phi_range, double _phi_slope_max, double _z0_max, bool _phi_reflect, bool _endcaps, bool _remove_intersecting_edges){
  //max_layer;
  vlids = (int**) malloc(2 * sizeof(void*));
  layer_pairs = (int**) malloc(2 * sizeof(void*));
  //n_layer_pairs;
  ...
  efficiency_by_hits = 0;
  efficiency_by_construction = 0;

  pt_min = _pt_min;
  n_eta_sections = _n_eta_sections;
  n_phi_sections = _n_phi_sections;
  eta_range = _eta_range;
  phi_range = _phi_range;
  phi_slope_max = _phi_slope_max;
  z0_max = _z0_max;
  phi_reflect = _phi_reflect;
  endcaps = _endcaps;
  remove_intersecting_edges = _remove_intersecting_edges;


}


  double* geometric_functions::calc_eta(double* r, double* z, size_t length){
  ...
  segment_T geometric_functions::create_paired_segment(hit_list_T hits1, hit_list_T hits2){
  ...
  segment_T geometric_functions::select_segment(segment_T seg, double phi_slope_cut, double z0_cut, bool intersect_cut){
   ...
   // WTF?????
    ...
    // currently, uses func argument 'intersect_cut' instead of object variable 'remove_intersecting_edges'
    if(intersect_cut){
    ...
    // currently, uses func arguments 'phi_slope_cut' and 'z0_cut' instead of object variables 'phi_slope_max' and 'z0_max'
    size_t phiCount = 0;
    size_t zCount = 0;
    size_t intersectCount = 0;
    for(size_t i = 0; i < seg->size; i++){
      if((abs(phi_slope[i]) < phi_slope_cut) && (abs(z0[i]) < z0_cut) && (intersected_layer[i] == false)){
        newLength++;
      }else{
        if(!(abs(phi_slope[i]) < phi_slope_cut)){
           phiCount++;
        }else{
          if(!(abs(z0[i]) < z0_cut)){
             zCount++;
  ...
  void** geometric_functions::construct_graph(hit_list_T* phits, double* feature_scale, size_t* pn_edges){
  /* Construct one graph (e.g. from one event) */
    ...
    if(pn_edges != NULL) *pn_edges = n_edges;
  ...
  int geometric_functions::vlid_hash(int vlid_volume, int vlid_layer){
    return vlid_layer*10 + vlid_volume; //10 is greater that max value of vlid_volume
  }

  void geometric_functions::set_vlids(){
  ...
  void geometric_functions::set_layer_pairs(){
  ...
  hit_list_T geometric_functions::select_hits(hit_list_T hits, size_t* pid, double* px, double* py, size_t psize){
  ...
  hit_list_T* geometric_functions::split_detector_sections(hit_list_T hits, double* phi_edges, double* eta_edges, size_t n_phi_edges, size_t n_eta_edges){
  ...
  double* geometric_functions::linspace(pair<double,double> range, size_t n_sections){
  ...
  void geometric_functions::soft_construct_graph(hit_list_T* phits, double* feature_scale, size_t* pn_edges){
  /* Construct one graph (e.g. from one event) */
    hit_list_T hits = *phits;
    ...
    for(size_t i = 0; i < n_layer_pairs; i++) {
      ...
      // Construct the segments
      segment_T allEdgeSegment = create_paired_segment(hits1, hits2);
      // currently cuts no edges based on z0 or phi_slope, but applies intersecting line cut
      // do we still want to aplly intersecting line cut?
      segment_T selectedEdgeSegment = select_segment(allEdgeSegment, 10000000000, 1000000000000, true);
